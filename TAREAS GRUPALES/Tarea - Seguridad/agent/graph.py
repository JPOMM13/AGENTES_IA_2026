...
from security.controls import sanitize as redact, request_signal as guard, public_text, clean_tree, deadline, TurnDeadline
...
class QuoteAgent:
    ...
    # Registra una operación y sus intentos; reintenta únicamente fallos de conexión o timeout hasta el presupuesto configurado.
    def _call(self, state, name, fn):
        state["tools_called"].append(name)
        # Un intento inicial + dos reintentos por defecto.
        for attempt in range(1, self.limits.retries + 2):
            state["attempts"][name] = attempt
            try:
                return fn()
            except (TimeoutError, ConnectionError):
                if attempt == self.limits.retries + 1:
                    raise
    ...
    def _understand(self, s):
        ...
        try:
            ...
            s["extracted"] = clean_tree(Extraction.model_validate(extracted).model_dump())
        except Exception:
    ...
    def _compose(self, s):
        ...
        text = f"Recomiendo {public_text(p['name'])} para {s['attendees']} asistentes; capacidad de {p['capacity']}."
        if q:
    ...
    def respond(self, message, previous=None, *, trusted_customer_id=None):
        ...
        prior = previous or {}
        count = prior.get("turn_count", 0)
        if not isinstance(message, str) or len(message) > self.limits.message_chars or count >= self.limits.session_turns:
            return {"response": "Límite de entrada o sesión alcanzado. Inicia una nueva sesión.",
                    "quote": None, "recommended_option": None, "tools_called": [], "messages": [],
                    "current_stage": "blocked", "needs_human": False, "handoff_reason": "resource_limit",
                    "turn_count": count, "validated_coverage": False, "validated_availability": False}
        if hasattr(self.interpreter, "reset_budget"):
            self.interpreter.reset_budget()
        s = clean_tree(deepcopy(prior))
        ...
        try:
            with deadline(self.limits.turn_seconds):
                result = self.graph.invoke(s)
        except TurnDeadline:
            self._handoff(s, "resource_limit")
            result = self._human(s)
        result["response"] = redact(result["response"])
        result["handoff"] = clean_tree(result.get("handoff"))
        result["messages"].append({"role": "assistant", "content": result["response"]})
        result["turn_count"] = count + 1
        while sum(len(m["content"]) for m in result["messages"]) > self.limits.history_chars:
